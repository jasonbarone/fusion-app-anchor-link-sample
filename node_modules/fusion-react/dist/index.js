...
function _prepare(element, context) {
  return prepareElement(element, context).then(([children, childContext]) => {
    return Promise.all(React.Children.toArray(children).map(child => _prepare(child, childContext)));
  });
}
...
function registerInjector(app) {
  ...
  const injectorPlugin = FusionApp.createPlugin({
    middleware: () => (ctx, next) => {
...
function withServices(deps, mapServicesToProps = defaultMap) {
  ...
  function renderConsumer(Component, props) {
    return React.createElement(InjectorContext.Consumer, null, inject => React.createElement(Component, resolve(inject, props)));
  }
