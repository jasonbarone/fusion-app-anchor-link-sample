...
function _prepare(element, context) {
  return prepareElement(element, context).then(function (_ref) {
    var children = _ref[0],
        childContext = _ref[1];
    return Promise.all(Children.toArray(children).map(function (child) {
...
function registerInjector(app) {
  ...
  var injectorPlugin = createPlugin({
...
function withServices(deps, mapServicesToProps) {
  ...
  function renderConsumer(Component$$1, props) {
    return createElement(InjectorContext.Consumer, null, function (inject) {
      return createElement(Component$$1, resolve(inject, props));
    });
  }
